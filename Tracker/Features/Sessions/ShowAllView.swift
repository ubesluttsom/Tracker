...
struct ShowAllView: View {
    ...
    var body: some View {
        NavigationView {
            List {
                ForEach(Array(viewModel.sessions.enumerated()), id: \.element.id) { index, session in
                    SessionListItem(viewModel: viewModel, session: session)
                        ...
                        .contextMenu {
                            if !isInCopyMode {
                                sessionContextMenu(for: session, at: index)
                            }
                        }
                }
            ...
            .navigationTitle(isInCopyMode ? "Select Source" : "All Timers")
            ...
            .safeAreaInset(edge: .top) {
                if isInCopyMode {
                    HStack {
                        ...
                        Text("Tap a session to copy its data into **\(copyTargetSession?.title ?? "")**")
                    }
                    .font(.subheadline)
    ...
    @ViewBuilder
    private func sessionContextMenu(for session: Session, at index: Int) -> some View {
        let above: Session? = index > 0 ? viewModel.sessions[index - 1] : nil
        let below: Session? = index < viewModel.sessions.count - 1 ? viewModel.sessions[index + 1] : nil
